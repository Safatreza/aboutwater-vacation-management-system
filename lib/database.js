...
if (!supabaseUrl || !supabaseKey || supabaseUrl.includes('placeholder')) {
  if (typeof window === 'undefined' && process.env.NODE_ENV !== 'development') {
    // Build time - create a mock client
    console.warn('⚠️ Using mock Supabase client for build');
    supabase = {
      from: () => ({
...
export const testConnection = async () => {
  try {
    ...
    if (typeof window === 'undefined' && (!process.env.NEXT_PUBLIC_SUPABASE_URL || process.env.NEXT_PUBLIC_SUPABASE_URL.includes('placeholder'))) {
      console.log('⚠️ Skipping connection test during build');
      return false;
    }
...
export const getEmployees = async () => {
  try {
    // Build-time fallback
    if (typeof window === 'undefined' && (!process.env.NEXT_PUBLIC_SUPABASE_URL || process.env.NEXT_PUBLIC_SUPABASE_URL.includes('placeholder'))) {
      console.log('🔄 Build time - using default employees');
...
export const getVacations = async () => {
  try {
    // Build-time fallback
    if (typeof window === 'undefined' && (!process.env.NEXT_PUBLIC_SUPABASE_URL || process.env.NEXT_PUBLIC_SUPABASE_URL.includes('placeholder'))) {
      console.log('🔄 Build time - using empty vacations');
      return [];
    }
...
export const initializeEmployeesInDatabase = async () => {
  try {
    ...
    if (typeof window === 'undefined' && (!process.env.NEXT_PUBLIC_SUPABASE_URL || process.env.NEXT_PUBLIC_SUPABASE_URL.includes('placeholder'))) {
      console.log('⚠️ Skipping employee initialization during build');
      return;
    }
...
export const generateExcelFromDatabase = async () => {
  try {
    console.log('🔄 Generating Excel from API endpoints...');

    // Dynamic import to avoid SSR issues
    const ExcelJS = (await import('exceljs')).default;

    // Fetch data from API endpoints instead of Supabase
    ...
    vacations.forEach(vacation => {
      ...
      vacationSheet.addRow({
        ...
        created: vacation.created_at ? new Date(vacation.created_at).toLocaleDateString('de-DE') : 'N/A'
      });
    });
    ...
    console.log('✅ Excel export from API endpoints completed successfully');
    return true;
  } catch (error) {
