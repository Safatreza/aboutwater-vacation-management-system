...
export const addVacation = async (vacationData) => {
  try {
    console.log('🔄 AddVacation: Starting request with data:', vacationData);
    ...
    console.log('📡 AddVacation: Response status:', response.status);
    console.log('📡 AddVacation: Response headers:', Object.fromEntries(response.headers.entries()));

    // Get response text first for debugging
    const responseText = await response.text();
    console.log('📡 AddVacation: Response text:', responseText);
    ...
    if (!response.ok) {
      ...
      console.error('❌ AddVacation: Server error:', errorData);
      throw new Error(`Server Error ${response.status}: ${JSON.stringify(errorData, null, 2)}`);
    }
    ...
    try {
      ...
      console.error('❌ AddVacation: Failed to parse success response:', parseError);
      throw new Error(`Failed to parse server response: ${responseText}`);
    }

    console.log(`✅ AddVacation: Success - ${result.vacation.days} days for employee ${result.vacation.employee_id}`);
    return result;
  } catch (error) {
    console.error('❌ AddVacation: Complete error details:', {
