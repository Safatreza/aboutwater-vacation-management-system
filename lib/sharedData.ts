// Shared data store to ensure consistency between APIs
// This fixes the critical issue where employees API and vacations API had separate data
...
// Single source of truth for vacation data
export let vacations: any[] = []

// Helper functions to update data consistently
export function addEmployee(employee: any) {
  employees.push(employee)
}

export function updateEmployee(employeeId: string, updates: any) {
...
export function addVacation(vacation: any) {
  vacations.push(vacation)
}

export function removeVacation(vacationId: string) {
  const index = vacations.findIndex(vac => vac.id === vacationId)
  if (index !== -1) {
    return vacations.splice(index, 1)[0]
  }
  return null
}
...
export function resetData() {
  vacations.length = 0
  // Reset employee vacation days to defaults if needed
}